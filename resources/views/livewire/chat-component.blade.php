<div class="flex flex-col min-h-screen bg-gray-100">
    ...
    <div class="h-16 bg-green-500 text-white flex items-center justify-between px-4 shadow z-10">
        <a href="/dashboard" class="flex items-center">
            <svg class="w-6 h-6 mr-2" fill="currentColor" viewBox="0 0 24 24">
                <path d="M9.41 11H17a1 1 0 0 1 0 2H9.41l2.3 2.3a1 1 0 1 1-1.42 1.4l-4-4a1 1 0 0 1 0-1.4l4-4a1 1 0 0 1 1.42 1.4L9.4 11z"/>
            </svg>
        </a>
        <div class="font-semibold text-lg truncate">{{ $user->name }}</div>
    ...
    <div class="flex-1 overflow-y-auto px-4 py-4 space-y-2">
        @forelse($comments as $comment)
            @php $isMe = $comment['sender'] === auth()->user()->name; @endphp
            <div class="flex {{ $isMe ? 'justify-end' : 'justify-start' }}">
                <div class="px-4 py-2 rounded-lg max-w-xs text-sm {{ $isMe ? 'bg-green-300 text-right' : 'bg-gray-200 text-left' }}">
                    @unless($isMe)
                        <div class="font-semibold text-xs text-gray-700">{{ $comment['sender'] }}</div>
                    @endunless
    ...
    <form wire:submit.prevent="sendMessage" class="bg-white border-t px-4 py-3 flex items-center gap-2">
        <textarea
            ...
            class="flex-1 resize-none rounded-full px-4 py-2 border border-gray-300 bg-gray-100 focus:outline-none focus:ring-2 focus:ring-green-400 text-sm"
        ></textarea>
